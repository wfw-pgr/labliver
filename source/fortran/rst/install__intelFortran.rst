...
* 別PC (WSL-Ubuntu24.04)にインストールした際は、プロキシの  :blue:`.wgetrc を設定しておらず` 、ブラウザから直接ダウンロード．
* 一回、失敗したためか、 .gpgが空のファイルになり、  :blue:`署名がとれず apt update できない` と怒られた．
* 一つ一つ命令を分解して、再実行、.gpgファイルを所定の位置において認証が通るように設定すると、問題なく成功．
