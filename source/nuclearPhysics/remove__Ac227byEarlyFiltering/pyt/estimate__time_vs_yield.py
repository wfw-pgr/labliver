...
def exchange__timeUnit( time=0.0, unit=None, direction="convert" ):
    ...
    cdict = { "y":365*24*60*60.0, "d":24*60*60.0, "h":60*60.0, "m":60.0, "s":1.0 }
...
def draw__figure( Data=None, settings=None, settingFile=None ):
    ...
    if ( "figure.num.additional" in settings ):
        for key,val in settings["figure.num.additional"].items():
            config[key] = val
    ...
    if ( "figure.act.additional" in settings ):
        for key,val in settings["figure.act.additional"].items():
            config[key] = val
