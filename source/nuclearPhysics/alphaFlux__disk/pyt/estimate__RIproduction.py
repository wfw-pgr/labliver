...
def write__results( Data=None, params=None, stdout="minimum" ):
    ...
    if ( params["results.yieldFile"] is not None ):
        ...
        spf.save__pointFile( outFile=params["results.yieldFile"], Data=Data_, \
...
def calculate__parameters( params=None ):
    ...
    if   ( params["target.N_atoms.from"].lower() == "activity" ):
        N_atoms  = params["target.activity.Bq"] / params["target.lambda.1/s"]
    elif ( params["target.N_atoms.from"].lower() == "mass"     ):
        N_atoms  = N_Avogadro * params["target.mass.g"] / params["target.g/mol"]
    else:
        sys.exit( "[estimate__RIproduction.py] target.N_atoms.from == ??" )
...
if ( __name__=="__main__" ):
    ...
    parser.add_argument( "--paramsFile", help="output file name." )        ## optional
