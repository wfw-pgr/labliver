...
# ========================================================= #
# ===  materials__fromDatabase.py                       === #
# ========================================================= #

def materials__fromDatabase( inpFile=None, outFile=None, extention=None ):

    default__settings = { "Density": 0.0, "Comment":"", \
                          "Color":"Blue", "CharacterSize":2.0 }
    ...
    if ( extention is None ): extention = ( os.path.splitext( inpFile ) )[1]
    ...
    if   ( extention.lower() == ".json" ):
        ...
        matDB = pd.read_csv( inpFile )
        matDB = matDB.fillna( default__settings )
    else:
        print( "[materials__fromDatabase.py] extention == {}??? ".format( extention ) )
    print( "[materials__fromDatabase.py] loaded inpFile :: {} ".format( inpFile ) )

    print( matDB )

    # ------------------------------------------------- #
    # --- [3] convert csv into dictionary           --- #
    # ------------------------------------------------- #
    if ( extention.lower() == ".csv" ):
        ...
        for index,item in matDB.iterrows():
    ...
    # ------------------------------------------------- #
    if ( extention.lower() == ".csv" ):
        jsonFile = inpFile.replace( extention, ".json" )
        with open( jsonFile, "w" ) as f:
            json5.dump( materials, f, indent=2 )
...
if ( __name__=="__main__" ):
    inpFile = "materials.csv"
    materials__fromDatabase( inpFile=inpFile )
